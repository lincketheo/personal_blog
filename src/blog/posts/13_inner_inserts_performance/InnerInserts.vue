  ...
  <article class="max-w-4xl mx-auto px-6 py-12 font-serif">
    ...
    <header class="mb-10">
      ...
      <p class="text-lg text-fg/70 leading-relaxed mt-4">
        ...
        deeply, and talk about where Numstore shines and where it shouldn't be
        used. I'll never intentionally hide a domain of performance in order to
      ...
      <div class="mt-6 space-y-4">
        <Definition term="SmartFiles">
          SmartFiles is Numstore's file abstraction that makes inner inserts and
          removals first-class operations - something the OS does not natively
          support - while providing ACID guarantees and write-ahead logging
          under the hood.
        </Definition>
        <Definition term="Inner Insert">
          An inner insert shifts existing bytes to the right and places new data
          at a given offset, growing the file. This is distinct from an inner
          write, which overwrites bytes in place without changing the file's
          length.
        </Definition>
        <Definition term="FALLOC_FL_INSERT_RANGE">
          A Linux <code class="font-mono text-sm">fallocate(2)</code> flag that
          instructs the kernel to insert a hole at a given offset, shifting
          subsequent data right. Requires XFS (Linux 4.1+) or ext4 (Linux 4.2+),
          and mandates that both the offset and insertion length be multiples of
    ...
    <section class="mb-10">
      ...
      >
        <li class="pl-3">
          ...
          will take to insert data into the middle of that file. This is not the
          case with SmartFiles. Inserting a fixed-size insert buffer of length
          <em>n</em> into the middle of a 100-byte file is comparable to
          inserting the same fixed-size insert buffer into a 20 GiB file - where
          regular file I/O would have exploded. Therefore, for workloads
          characterized by high data volume and frequent interior inserts,
          SmartFiles offers a substantial advantage.
        </li>
        <li class="pl-3">
          Generally, as the length of the fixed-size insert buffer increases (as
          in case 1), SmartFiles doesn't offer any performance benefit over raw
          file I/O, and is around 3× slower due to durability overhead and
          logging. Workloads that predominantly write large, contiguous payloads
          to interior offsets should not expect a performance gain from
          SmartFiles.
        </li>
        <li class="pl-3">
          SmartFiles is functionally comparable to
          <Code>fallocate(2)</Code> with <Code>FALLOC_FL_INSERT_RANGE</Code>,
          but with fewer constraints. Benchmarks show SmartFiles to be more
          consistently performant at higher file sizes.
          <Code>FALLOC_FL_INSERT_RANGE</Code> imposes significant restrictions
          in practice: it requires an ext4 filesystem and mandates that both the
          insertion length and offset be integer multiples of the filesystem
          block size. SmartFiles carries no such constraints, making it more
          portable and easier to reason about in general-purpose contexts.
        </li>
      </ol>
      ...
      <div class="my-6">
        ...
        <p class="mt-2 text-fg/50 text-sm leading-relaxed">
          ...
          small). As file size increases, SmartFiles takes relatively the same
          amount of time to insert data into the interior of the file
...
[ 0 | 9 | 10 | 11 | 4 | 5 | 6 | 7 | 8 ]</pre
        ...
        <p class="text-fg/75 leading-relaxed mb-4">
          ...
          common use case. So now I want to insert the same data (<Code
            >[ 9 | 10 | 11 ]</Code
          >) into position 1 of the file like this:
        </p>
      ...
      <section>
        ...
        >
          <li class="pl-3">
            ...
            file into two segments - before and after our offset.
          </li>
          <li class="pl-3">
            Write our data to the end of this truncated file.
          </li>
        ...
        >
{
    ...
    i_fsync (fd);
        ...
        <p class="text-fg/75 leading-relaxed mb-4">
          ...
          of 4096 bytes, we never need to hold more than 4096 bytes in memory at
          once. However, we still "touch" all the bytes to the right of our
          insertion point. As the file grows without bound, we always need to
          touch the right half of our file in some way, unless we have an
    ...
    <section class="mb-10">
          ...
          >
            <li class="pl-2">
              ...
              <ul class="list-disc list-inside pl-4 mt-1 space-y-1">
                <li>
                  You can never write 10 bytes if you requested to write 100
                  bytes.
                </li>
                <li>
                  If you pull the plug while writing data, Numstore reboots and
            ...
            <li class="pl-2">
              ...
              <ul class="list-disc list-inside pl-4 mt-1 space-y-1">
                <li>
                  You never need to care about the tail of your file when doing
                  an inner insert.
                </li>
              </ul>
            </li>
            <li class="pl-2">
              Numstore uses less code. Compare the naive implementations above
              to Numstore:
      ...
      <section>
        ...
        >
          ...
          <li class="pl-3">
            <Code>fallocate</Code> with <Code>FALLOC_FL_INSERT_RANGE</Code>
          </li>
      ...
      <section>
          ...
          <table class="w-full text-sm font-mono">
            <thead>
              <tr class="border-b border-text/10 bg-text/[0.03]">
                <th
                  class="text-left px-4 py-2 text-muted font-normal tracking-widest uppercase text-xs"
                >
                  Parameter
                </th>
                <th
        ...
        <p class="text-fg/75 leading-relaxed mb-4">
          ...
          the write-ahead log, plus the flushed page to disk on the eviction
          path.
        </p>
        ...
        <div class="my-6">
          ...
          <p class="mt-2 text-fg/50 text-sm leading-relaxed">
            ...
            relatively small). As insertion size increases, SmartFiles balloons
            at a higher rate than naive file I/O - because SmartFiles has
            substantial interior machinery for writing a contiguous array of
            bytes, whereas a plain write is a single system call.
          </p>
        </div>

        <div
          class="my-6 border border-text/10 rounded bg-text/[0.03] px-5 py-4"
        >
          <p
      ...
      <section>
        ...
        <p class="text-fg/75 leading-relaxed mb-4">
          ...
          faster, but it tends to grow at larger file sizes as well. If anyone
        ...
        <p class="text-fg/75 leading-relaxed mb-4">
          ...
          grows (larger files), naive file I/O grows proportionally, but
          SmartFiles remains relatively stable.
        </p>
    ...
    <section class="mb-10">
      ...
      <div class="my-6">
        ...
        <p class="mt-2 text-fg/50 text-sm leading-relaxed">
          ...
          time is spent between the <Code>fsync</Code>, <Code>pread</Code>, and
          <Code>pwrite</Code> calls.
        </p>
      </div>
      ...
      <div class="my-6">
        ...
        <p class="mt-2 text-fg/50 text-sm leading-relaxed">
          ...
          is spent inside
          <Code>pgr_new</Code> - we're constantly creating new pages and
          therefore reading the free-space map frequently (something I intend to
          optimize). Significant time is also spent in
          <Code>pgr_release_with_log</Code>, which writes data to the WAL and
          triggers a WAL flush.
        </p>
      </div>
      ...
      >
        <li class="pl-3">
          ...
          <Code>walos_write_all</Code> and <Code>pgr_flush_unsafe</Code>. When I
          tried a hardware-accelerated instruction, this time dropped
          noticeably. The next version of Numstore will optimize this path.
        </li>
        <li class="pl-3">
          A lot of time is spent reading the free-space map in
          <Code>pgr_get_writable</Code>. This is an artifact of the pager insert
          pattern. A basic LRU caching mechanism failed to trigger on
          long-running inserts. This is an inconsistency and will be fixed in
          the next release.
        </li>
      </ol>
