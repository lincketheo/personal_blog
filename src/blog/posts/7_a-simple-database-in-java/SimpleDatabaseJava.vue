...
<template>
  <article class="max-w-2xl mx-auto px-6 py-12 font-serif">
    ...
    <section class="mb-8">
      ...
      <p class="text-text/75 leading-relaxed mb-4">Our data is just a hash map of files inside of a folder:</p>
      <pre class="bg-surface border-l-4 border-red/40 px-5 py-4 overflow-x-auto rounded-r my-5"><code class="font-mono text-sm text-text/85 leading-relaxed">Database\
    File1
    File2
    File3</code></pre>
    ...
    <section class="mb-8">
      <h2 class="text-xl font-bold text-text mt-10 mb-3">Handling Concurrency: Just Lock the Whole Thing</h2>
      <p class="text-text/75 leading-relaxed mb-4">
    ...
    <section class="mb-8">
      <h2 class="text-xl font-bold text-text mt-10 mb-3">The DO UNDO REDO Protocol</h2>
      <p class="text-text/75 leading-relaxed mb-4">
      ...
      <Definition term="DO UNDO REDO">
        <ul>
          ...
          <li><strong>UNDO</strong>: use the saved record to reverse an operation that shouldn't have happened (rollback or crash recovery)</li>
          <li><strong>REDO</strong>: use the saved record to replay an operation that did happen but whose effects were lost (e.g. crash after commit but before the data was flushed to disk)</li>
        </ul>
    ...
    <section class="mb-8">
      <h2 class="text-xl font-bold text-text mt-10 mb-3">How a Transaction Works</h2>
      <p class="text-text/75 leading-relaxed mb-4">A transaction modifies files by doing the following:</p>
      <ol class="list-decimal list-outside pl-5 space-y-2 text-text/75 leading-relaxed mb-4">
        <li>Make a backup file called <Code>File1.undo</Code> if there isn't already one, else goto step 3</li>
    ...
    <section class="mb-8">
      ...
      <ol class="list-decimal list-outside pl-5 space-y-2 text-text/75 leading-relaxed">
    ...
    <section class="mb-8">
      ...
      <ol class="list-decimal list-outside pl-5 space-y-2 text-text/75 leading-relaxed mb-4">
        ...
        <li>Otherwise, go through all the <Code>.undo</Code> files and replace the original files with them, then delete the <Code>.undo</Code> and <Code>.crc</Code> files. There may be one file that has an incomplete CRC, meaning our database crashed while writing a CRC. If there's more than one — then our database is corrupt.</li>
      </ol>
    ...
    <section class="mb-8">
      ...
      <p class="text-text/75 leading-relaxed mb-4">
        ...
        <Code>File1</Code> twice, I'd have <Code>File1.undo.1</Code> and <Code>File1.undo.2</Code>.
        But in write ahead logging, we don't technically need to keep all the records.
      </p>
    ...
    <footer class="mt-12 pt-6 border-t border-border">
      <p class="text-muted italic text-sm">Anyways, I thought it was a nice little database and wanted to share it.</p>
    </footer>
