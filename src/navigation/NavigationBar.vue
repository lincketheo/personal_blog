  ...
  <nav class="bg-surface px-8 py-5">
    <div class="max-w-2xl mx-auto flex items-center justify-between">
      ...
      <ul class="hidden sm:flex gap-8">
        <li v-for="link in links" :key="link.name">
          <a
              ...
              :class="currentLink === link.name ? 'text-red font-semibold' : 'text-muted hover:text-text transition-colors'"
      ...
      <button
          ...
          @click="toggle"
      >
        <font-awesome-icon :icon="open ? ['fas', 'xmark'] : ['fas', 'bars']" class="text-xl"/>
      </button>

    </div>

    <!-- Mobile dropdown -->
    <div v-if="open" class="sm:hidden max-w-2xl mx-auto pt-4 pb-1 border-t border-border mt-4">
